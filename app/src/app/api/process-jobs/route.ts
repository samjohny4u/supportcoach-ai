...
1. Timestamp and Response Time Analysis:
   ...
   - TIMESTAMP CITATION LIMIT — CRITICAL: A coaching message should contain no more than 3 to 4 timestamp citations total. Timestamps are evidence, not decoration. Only cite a timestamp when the specific moment in time is the coaching point itself — for example, when a delay caused disengagement, or when a sequence of messages shows a pattern. Do NOT cite timestamps when praising routine positive behaviors. Do NOT cite timestamps when describing what the agent said if the timing is irrelevant. Do NOT cite a timestamp just to show you read the transcript. Ask yourself before every timestamp citation: "Does knowing the exact time of this message change the coaching point?" If the answer is no, remove the timestamp and just describe the behavior. A coaching message with 8 or more timestamps is always wrong — it means timestamps are being used as filler, not as evidence. IMPORTANT: Reducing timestamps does NOT mean reducing detail, specificity, or length. Every coaching point must still quote specific transcript evidence, describe exactly what happened, and provide actionable improvement suggestions. The Summary section must always use bullet points — never a paragraph. Coaching quality and depth must be preserved. Timestamps are evidence, not decoration. Only cite a timestamp when the specific moment in time is the coaching point itself — for example, when a delay caused disengagement, or when a sequence of messages shows a pattern. Do NOT cite timestamps when praising routine positive behaviors. Do NOT cite timestamps when describing what the agent said if the timing is irrelevant. Do NOT cite a timestamp just to show you read the transcript. Ask yourself before every timestamp citation: "Does knowing the exact time of this message change the coaching point?" If the answer is no, remove the timestamp and just describe the behavior. A coaching message with 8 or more timestamps is always wrong — it means timestamps are being used as filler, not as evidence.
