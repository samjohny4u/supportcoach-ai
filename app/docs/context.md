...
- No active blockers
- AI Weekly Team Summary empty in production ("AI summary could not be generated yet") — DIAGNOSED August 26, 2026, awaiting owner config fix. The route works (verified by direct POST to https://www.supportcoach.io/api/team-summary → 200 with a full summary). The dashboard page self-fetches via `NEXT_PUBLIC_SITE_URL || "http://localhost:3000"` (dashboard/page.tsx getTeamAISummary) — the var is set locally but missing/wrong in Vercel, so production fetches localhost inside the lambda and fails instantly. FIX (owner): Vercel → project → Settings → Environment Variables → set `NEXT_PUBLIC_SITE_URL` = `https://www.supportcoach.io` (https, www, no trailing slash) for Production, then redeploy. Optional code hardening if wanted later: fall back to `https://${VERCEL_URL}`, or drop the HTTP self-hop entirely.
