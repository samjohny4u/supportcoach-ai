...
## CURRENT TASK
- **Phase 3 (August 2026 bug fixes and hardening): Tasks 1-3, 5-20, 22 DONE and owner-verified in production August 26, 2026.** One marathon day: see PHASE 3 TASKS in codex-orchestration.md for every as-built entry. Owner verified end-to-end in production: repeat-coaching loop (upload → follow-through cards → Since Last Coaching section → repeat counters), short-chat proportional coaching, transcript view, Product Issues page, coaching digest button, settings save, AI Weekly Team Summary.
...
- SECURITY observation from the same test: `/api/team-summary` is UNAUTHENTICATED and performs an OpenAI call per POST — anyone can curl it and burn OpenAI spend (same class as the unauthenticated /api/process-jobs GET already in the scale backlog). Add both to the pre-launch auth pass.
...
- `src/app/security/page.tsx` — Security and IT Review page (public, in sitemap, added Aug 20 2026). Written for IT/security reviewers evaluating the Chrome extension: manifest permissions, the exact four-field check payload, subprocessors, and Chrome Enterprise force-install instructions. EVERY CLAIM WAS VERIFIED AGAINST THE `support-coach-extension` REPO SOURCE, not written from memory — re-verify before editing, and never soften or embellish it. Load-bearing facts: the manifest requests only `storage` plus five help-desk host patterns; the API receives only `draft_text`/`triggered_rules`/`local_score`/`draft_hash`; the customer's messages never leave the browser; no draft text is persisted. Section 7 deliberately states the gaps (no SOC 2, no SSO, no data region) — do not remove it, an IT page that claims nothing is missing reads as marketing. Linked from the /extension footer and privacy §9.
...
"I'm continuing development of SupportCoach AI. Read docs/RULES.md and docs/CONTEXT.md for current status. The app is live at supportcoach.io. Paddle billing verified end-to-end March 25, 2026. **Phase 3 (August 26, 2026 marathon) is essentially complete — Tasks 1-20 and 22 are DONE and owner-verified in production**: repeat-coaching loop works end to end (Operator-header agent guess, Since Last Coaching section in the coaching message, repeat counters, override calibration feedback, copy summary button), short-chat proportional coaching, transcript view at page bottom, Product Issues rollup, on-demand Coaching Digest, atomic job claim, settings NEXT_REDIRECT fixed, NEXT_PUBLIC_SITE_URL added in Vercel (team summary now works in production for the first time). **NEXT UP: Phase 3 Task 21 (coaching-point dedup via recurrence_of), owner-scheduled for ~August 28** — spec in codex-orchestration.md. Also open: Phase 2 Task 6b (agent coaching history view — read the AS-BUILT vs SPEC table first), Task 9's bi-weekly automation (blocked on cron decision), plan gating enforcement, dashboard UI polish, the scale-readiness backlog (unauthenticated /api/process-jobs and /api/team-summary need auth before external launch). Test org is temporarily plan='enterprise' with a synthetic subscription — revert SQL in CONTEXT.md."

**Landing /extension hero + wordmark update (Sep 16, 2026, commit c4d14a5, pushed — live on Vercel):**
- H1 replaced with the owner-approved PRIMARY positioning line from the extension repo's docs/POSITIONING.md §3: "Your help desk's AI rewrites replies when agents ask. Support Coach speaks up when they don't." (setup sentence dimmed, punchline white; clamp max 72px -> 56px since it is two sentences). It pre-empts the "our help desk already does this" objection in the headline — the old "Prevent bad support replies before they happen." predated the Aug 20 positioning work.
- Subhead now carries the SECONDARY line in its designated slot plus the mechanism: "Catches the reply that escalates — three seconds before your customer reads it. Support Coach watches the draft as your agent types and suggests a complete, ready-to-send rewrite."
- SUPPORTCOACH wordmark 11px -> 15px, letter-spacing 0.3em -> 0.25em: it rendered smaller than the Sign In link next to it. No logo image exists; the text is the brand mark.
- Owner-approved on a local preview before push. Copy is verbatim from POSITIONING.md — no invented claims, no numbers.

**Funnel attribution + website analytics (Sep 20, 2026, committed NOT pushed — pre-paid-test prerequisite, brief from the extension repo's marketing thread):**
- WEBSITE PAGEVIEWS ARE CAPTURED SERVER-SIDE in middleware.ts (event website_pageview to PostHog project 561046): /privacy §11 publicly promises "no analytics script or analytics cookie is loaded on this site", so a browser-side sender — even cookieless — was ruled out. Edge capture: public paths only (/, /extension, /blog/*, /privacy, /security, /terms, /refund, /support — added to the matcher with an early return BEFORE any auth/subscription logic, so their previous no-middleware behavior is preserved), document navigations only (prefetch/RSC/bot-UA skipped), allowlisted utm_*/ref/gclid, referrer DOMAIN only (own domains excluded), no visitor IP forwarded, geo disabled, anonymous. The embedded phc_ token is PostHog's PUBLISHABLE write-only project token (the class every PostHog browser snippet exposes publicly) — not a secret.
- FIRST-TOUCH ATTRIBUTION: src/lib/attribution.ts (sanitize-by-rejection, localStorage 90 days, first touch wins, re-sanitized on read) + AttributionCapture in the root layout (stores locally, sends NOTHING) + /extension's three signup CTAs decorate the admin.supportcoach.io/signup link with the stored params after hydration — the cross-domain handoff into the extension repo's signup, which sanitizes again and stores/forwards (organizations.attribution + org_signed_up).
- /privacy: dated "How You Found Us" paragraph added under Usage Data; Last updated -> September 20, 2026. Ships in the SAME push as the capture.
- NEW TEST SUITE, ZERO DEPENDENCIES: tests/attribution.test.mjs under node --test (npm run test; Node's native TS type-stripping) — 5/5: rejection-not-repair, allowlist parse + full-referrer-never-survives, own-domain exclusion, first-touch-wins/expiry, CTA decoration. No lockfile change. tsc clean, next build clean (57 pages).
- PUSH ORDER (owner): extension repo first (backend accepts + stores attribution; its SQL docs/sql/2026-09-attribution.sql runs in Supabase before that), THEN this repo. Smoke test after both: /extension?utm_source=smoketest&utm_campaign=x -> Start Free Trial -> throwaway signup -> verify org row + org_signed_up + website_pageview all carry utm_source=smoketest.
