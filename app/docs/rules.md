...
OpenAI, TypeScript, Tailwind CSS.
...
18. NEVER store payment information in the database. All payment processing goes through Stripe (when implemented).
19. NEVER expose `SUPABASE_SERVICE_ROLE_KEY` or `OPENAI_API_KEY` in client-side code or browser-accessible responses.
